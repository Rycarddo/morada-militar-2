...
export default function Hero() {
  return (
    <section id="hero" className="relative min-h-screen bg-surface-page flex items-center overflow-hidden">
      {/* Warm ambient glow — like a hearth in the distance */}
      <div className="absolute inset-0 pointer-events-none" style={{
        background: "radial-gradient(ellipse 60% 50% at 75% 60%, rgba(123,79,46,0.08) 0%, transparent 65%), radial-gradient(ellipse 40% 60% at 20% 80%, rgba(232,160,32,0.04) 0%, transparent 60%)"
      }} />
      ...
      <div className="max-w-7xl mx-auto px-6 w-full pt-24 pb-16 z-10 relative">
        <div className="grid grid-cols-1 lg:grid-cols-[48fr_52fr] gap-8 lg:gap-16 items-center">
          ...
          <div className="flex flex-col gap-7">
            ...
            {/* Headline */}
            <motion.h1 custom={1} initial="hidden" animate="show" variants={variants}
              className="font-display font-bold leading-[0.9] tracking-[-0.02em] text-text-primary"
              style={{ fontFamily: "var(--font-playfair, 'Playfair Display', serif)", fontSize: "clamp(52px, 7.5vw, 88px)" }}>
              Sua próxima
              <br />
              <span className="text-gold italic" style={{ letterSpacing: "-0.03em" }}>
                morada.
              </span>
            </motion.h1>

            {/* Subtitle */}
            <motion.p custom={2} initial="hidden" animate="show" variants={variants}
              className="text-text-secondary leading-relaxed max-w-[440px]"
              style={{ fontFamily: "var(--font-sans, 'DM Sans', sans-serif)", fontSize: "clamp(15px, 1.4vw, 18px)" }}>
              Residência de alto padrão exclusiva para militares em Manaus.
              Conforto, segurança e comunidade — como deve ser.
            </motion.p>

            {/* Divider line */}
            <motion.div custom={3} initial="hidden" animate="show" variants={variants}
          ...
          <motion.div custom={5} initial="hidden" animate="show" variants={variants}
            ...
            <div className="relative h-full rounded-card overflow-hidden bg-surface-mid">
              <Image src={heroImg} alt="Sala de estar — Morada do Militar" fill className="object-cover" priority />
              {/* Dark vignette overlay */}
              <div className="absolute inset-0" style={{
                background: "linear-gradient(to top, rgba(0,0,0,0.6) 0%, rgba(0,0,0,0.1) 50%, transparent 100%)"
              }} />
              {/* Floating info card */}
              <div className="absolute bottom-5 left-5 right-5 backdrop-blur-sm rounded-card p-5"
                style={{ background: "rgba(13,15,15,0.90)", border: "1px solid rgba(123,79,46,0.40)" }}>
                <div style={{ color: "rgba(255,255,255,0.50)", fontSize: 10, fontWeight: 600, letterSpacing: "0.2em", textTransform: "uppercase", marginBottom: 6, fontFamily: "var(--font-sans, 'DM Sans', sans-serif)" }}>Disponível agora</div>
                <div style={{ color: "#ffffff", fontSize: 14, fontWeight: 600, marginBottom: 4, fontFamily: "var(--font-sans, 'DM Sans', sans-serif)" }}>Rua 55, 189 — Japiim, Manaus</div>
                <div style={{ color: "rgba(255,255,255,0.45)", fontSize: 12, fontFamily: "var(--font-mono, 'JetBrains Mono', monospace)" }}>45 m² · 1 quarto</div>
              </div>
            </div>
          </motion.div>
        </div>
